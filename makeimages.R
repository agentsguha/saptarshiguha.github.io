...
ii <- sprintf("%s\n",paste(unlist(lapply(1:nrow(m),function(i){
    ...
    if(convert) {
        ...
        conv2 <- sprintf("convert %s -quality 98 -resize %sx%s %s", l$i, as.integer(l$w/Fw), as.integer(l$h/Fh), gsub("DSC","st-", basename(l$i)))
        print(conv)
        print(conv2)
        system(conv);system(conv2)
    }
    sprintf('<div class="item" data-w="%s" data-h="%s">\n\t<div class="img"><a href="{{ site.url }}/images/photos/%s/%s"><img src="{{ site.url }}/images/blank.gif" data-src="{{ site.url }}/images/photos/%s/%s"></a></div>\n</div>', as.integer(l$w/Fw), as.integer(l$h/Fh),  PATH,if(convert) gsub("DSC","t-", basename(l$i)) else basename(l$i),PATH,if(convert) gsub("DSC","st-", basename(l$i)) else basename(l$i))
})),collapse='\n'))
cat(ii)
cat("\n")
